...
//---------------- RESOURCES LIST ----------------------
extern coap_resource_t aircond_res;
//------------------------------------------------------


/* Declare and auto-start the server and registrator processses */
PROCESS(aircond_registration_process, "Registration process for the air-conditioning");
AUTOSTART_PROCESSES(&aircond_registration_process);
...
uint8_t registered_aircond = 0;


// Define a handler to handle the response from the server
void response_handler_aircond(coap_message_t* response) 
{
	uint8_t resp_code;	
...
PROCESS_THREAD(aircond_registration_process, ev, data)
{
	...
	coap_activate_resource(&aircond_res, "aircond");
	...
	while(!registered_aircond) {
	  	...
	  	// Set the payload
	  	const char msg[] = "{\"a\" : \"register\" , \"t\" : \"actuator\" , \"st\" : \"aircond\" , \"p\" : \"/aircond\"}";
	  	...
	  	COAP_BLOCKING_REQUEST(&server_ep, request, response_handler_aircond);
