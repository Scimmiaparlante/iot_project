...
AUTOSTART_PROCESSES(&temp_registration_process, &server_process);
...
uint8_t registered_temperature = 0;


// Define a handler to handle the response from the server
void response_handler_temperature(coap_message_t* response) 
{
	LOG_INFO("Temperature sensor - Handling registration response...\n");
	
	if(response == NULL) {
		LOG_INFO("Temperature sensor - Request timed out\n");
		return;
	}

	if(response->code == CHANGED_2_04) {
		LOG_INFO("Temperature sensor - Registration successful\n");
		registered_temperature = 1;
	} else if (response->code == BAD_REQUEST_4_00) {
		LOG_INFO("Temperature sensor - Registration failed: BAD REQUEST\n");
	} else {
		LOG_INFO("Temperature sensor - Registration failed: unknown error: %x\n", response->code);
	}
}
...
PROCESS_THREAD(temp_registration_process, ev, data)
{
	...
	while(!registered_temperature) {
	  	...
	  	LOG_INFO("Temperature sensor - Issuing registration request...\n");
	  	COAP_BLOCKING_REQUEST(&server_ep, request, response_handler_temperature);
