...
//---------------- RESOURCES LIST ----------------------
extern coap_resource_t patientalarm_res;
//------------------------------------------------------


/* Declare and auto-start the server and registrator processses */
PROCESS(patientalarm_registration_process, "Registration process for the fire alarm");
AUTOSTART_PROCESSES(&patientalarm_registration_process);
...
uint8_t registered_patientalarm = 0;


// Define a handler to handle the response from the server
void response_handler_patientalarm(coap_message_t* response) 
{
	uint8_t resp_code;	
...
PROCESS_THREAD(patientalarm_registration_process, ev, data)
{
	...
	coap_activate_resource(&patientalarm_res, "patientalarm");
	...
	while(!registered_patientalarm) {
	  	...
	  	// Set the payload
	  	const char msg[] = "{\"a\" : \"register\" , \"t\" : \"actuator\" , \"st\" : \"patientalarm\" , \"p\" : \"/patientalarm\"}";
	  	...
	  	COAP_BLOCKING_REQUEST(&server_ep, request, response_handler_patientalarm);
