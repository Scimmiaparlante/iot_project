...
#define HB_PUBLISH_INTERVAL 		(3*CLOCK_SECOND)
...
AUTOSTART_PROCESSES(&heartbeat_registration_process, &server_process, &alarm_registration_process);
...
PROCESS_THREAD(server_process, ev, data) 
{
	...
	//timer to send the request again in case of failure
	etimer_set(&periodic_timer, HB_PUBLISH_INTERVAL);
