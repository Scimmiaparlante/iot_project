...
AUTOSTART_PROCESSES(&heartbeat_registration_process, &server_process, &alarm_registration_process, &bloodpressure_registration_process);
...
{
	LOG_INFO("Alarm - Handling registration response...\n");
	
	if(response == NULL) {
		LOG_INFO("Alarm - Request timed out\n");
		return;
	}

	if(response->code == CHANGED_2_04) {
		LOG_INFO("Alarm - Registration successful\n");
		registered_alarm = 1;
	} else if (response->code == BAD_REQUEST_4_00) {
		LOG_INFO("Alarm - Registration failed: BAD REQUEST\n");
	} else {
		LOG_INFO("Alarm - Registration failed: unknown error: %x\n", response->code);
	}
}
...
PROCESS_THREAD(alarm_registration_process, ev, data)
{
	...
	while(!registered_alarm) {
	  	...
	  	// Issue the request in a blocking manner 
	  	// The function returns when the request has been sent (ack received)
	  	LOG_INFO("Alarm - Issuing registration request...\n");
...
PROCESS_THREAD(server_process, ev, data) 
{
	...
	coap_activate_resource(&alarm_res, "bloodpressure");
