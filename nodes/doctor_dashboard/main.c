...
//---------------- RESOURCES LIST ----------------------
extern coap_resource_t dashboard_res;
//------------------------------------------------------


/* Declare and auto-start the server and registrator processses */
PROCESS(dashboard_registration_process, "Registration process for the dashboard");
AUTOSTART_PROCESSES(&dashboard_registration_process);
...
uint8_t registered_dashboard = 0;


// Define a handler to handle the response from the server
void response_handler_dashboard(coap_message_t* response) 
{
	uint8_t resp_code;	
...
PROCESS_THREAD(dashboard_registration_process, ev, data)
{
	...
	coap_activate_resource(&dashboard_res, "dashboard");
	...
	while(!registered_dashboard) {
	  	...
	  	// Set the payload
	  	const char msg[] = "{\"a\" : \"register\" , \"t\" : \"actuator\" , \"st\" : \"dashboard\" , \"p\" : \"/dashboard\"}";
	  	...
	  	COAP_BLOCKING_REQUEST(&server_ep, request, response_handler_dashboard);
